...
**/
/****************************************************************/
#ifndef USB_CDC_IF_EX_H__
	#define USB_CDC_IF_EX_H__
...
** \note \c get_CDC_Line_State can be used instead (if not willing to use callback) to check port current status
...
** \note Call CDC_Control_Set_Line_State in usbd_cdc_if.c \c CDC_Control_FS function, CDC_SET_CONTROL_LINE_STATE case
...
** \note Call CDC_Control_Set_Line_Coding in usbd_cdc_if.c \c CDC_Control_FS function, CDC_SET_LINE_CODING case
** \param[in] pbuf - Pointer to message data
**/
void CDC_Control_Set_Line_Coding(const uint8_t * const pbuf);

/*!\brief CDC Get Line Coding message handler
** \note Call CDC_Control_Get_Line_Coding in usbd_cdc_if.c \c CDC_Control_FS function, CDC_GET_LINE_CODING case
